        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre Valarya, vous avez besoin d\'une installation 64 bits de Java 8. Voulez-vous que nous en installions une copie ?',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation x64 de Java 8 est requise pour lancer le jeu.<br><br>Référez vous au <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">guide</a> pour suivre les instructions d installations de Java.',
                            'I Understand',
                            'Go Back'
                        )
                        setOverlayHandler(() => {
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur de Lancement', err.message || 'Ouvrez la console (CTRL + Shift + i) pour plus de détails.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('Erreur de Lancement', 'Ouvrez la console (CTRL + Shift + i) pour plus de détails.')
        }
    })
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'version':
                    ...
                    loggerLaunchSuite.log('Données de la version chargée.')
                    setLaunchDetails('Validation des données..')
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Asset Validation Complete')
                    setLaunchDetails('Validation des bibliothèques..')
                    break
                case 'libraries':
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        ...
                        showLaunchFailure(
                            'Erreur de téléchargement',
                            'Veuillez ouvrir la console (CTRL + Shift + i) pour plus de détails.'
                        )
                    }
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                loggerLaunchSuite.error('Erreur durant la vérification:', m.result)

                loggerLaunchSuite.error('Erreur de lancement', m.result.error)
                showLaunchFailure('Erreur de lancement', 'Veuillez ouvrir la console (CTRL + Shift + i) pour plus de détails.')
            ...
            if(login && allGood) {
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        loggerLaunchSuite.error('Le lancement du jeu a échoué, le fichier principal, LaunchWrapper n\'a pas été téléchargé correctement.')
                        showLaunchFailure('Le lancement du jeu a échoué', 'Le fichier principal, LaunchWrapper, n\'a pas été téléchargé correctement. Par conséquent, le jeu ne peut pas être lancé.<br><br>Pour résoudre ce problème, désactivez temporairement votre logiciel antivirus et relancez le jeu.<br><br>Si vous avez le temps, rejoignez le <a href="https://discord.gg/cB8aTYbybq">discord</a> et ouvrez un ticket pour nous faire savoir quel logiciel antivirus vous utilisez. Nous allons les contacter et essayer d\'arranger les choses.')
                    }
                }
                ...
                try {
                    ...
                    loggerLaunchSuite.error('Error during launch', err)
                    showLaunchFailure('Erreur durant le lancement', 'Veuillez vérifier la console (CTRL + Shift + i) pour plus de détails.')
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Chargement des news'
        let dotStr = '..'
